...
const ClothingCard = ({ item, onSelect, isSelected }) => {
  ...
  return (
    ...
    >
      <Card.Img variant="top" src={`${process.env.NEXT_PUBLIC_API_URL}/images/patagonia.jpg`} alt={item.category || 'Image'} />
      <Card.Body>
        <Card.Title>{item.brand || 'No brand'} {item.color || 'No color'} {item.category || 'No category'}</Card.Title>
        <Card.Text>
